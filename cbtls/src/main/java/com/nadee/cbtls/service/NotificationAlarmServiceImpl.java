...
public class NotificationAlarmServiceImpl implements NotificationAlarmService {
  ...
  @Override
  public Map<String, Object> setNotificationAlarm(NotificationAlarmDTO notificationAlarmDTO)
      throws Exception {
    ...
    if (StringUtils.isNotEmpty(notificationAlarmDTO.getSystemUserMobileDevice())) {
      SystemUserMobileDevice systemUserMobileDevice = systemUserService
          .getSystemUserMobileDeviceByUniqueId(notificationAlarmDTO.getSystemUserMobileDevice());
      if (systemUserMobileDevice == null) {
        systemUserMobileDevice = systemUserService
            .createMobileUser(notificationAlarmDTO.getSystemUserMobileDevice(), null);
      }
      ...
      if (!(notificationAlarmDTO.getUpdatedUser() == 0)) {
        systemUser =
            commonDAO.getEntityById(SystemUser.class, notificationAlarmDTO.getUpdatedUser());
      }
      if (systemUser == null) {
